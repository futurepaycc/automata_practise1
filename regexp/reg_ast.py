...
class NFARulebook(object):
    ...
    def _follow_rules_for(self, state, character):
        return [rule.follow() for rule in self._rules_for(state, character)]

    def _rules_for(self, state, character):
        return [rule for rule in self.rules if rule.applies_to(state, character)]

    def next_states(self, states, character):
        nexts = []
        for state in states:
            nexts += self._follow_rules_for(state, character) 
        return set(nexts)
...
##test
# -----------------------------------------------------------模式AST,  模式就是 ϵ,equ 匹配的 ∙,*,| 组合运算, 这里有点函子的意思么?
pattern = Repeat(
            Choose(
