""" 
来源: http://www.nightmare.com/rushing/python/nfa.py
说明: 缺少dfa.py应该也可以简单运行
问题: 测试代码貌似有bug
优点: 实现了regexp->enfa->dfa, 含closure的move转换，算法比较完全
 """
...
def nary_to_binary (exp):
    if type(exp) == type(''):
        if len(exp) > 1:
            return nary_to_binary ((CONCAT,) + tuple(list(exp)))
        else:
            return exp
    else:
        op = exp[0]
        args = list(map (nary_to_binary, exp[1:]))
        if len(args) < 3:
...
def pprint_regexp (exp):
    if type(exp) == type(''):
        ...
        op = exp[0]
        args = list(map (pprint_regexp, exp[1:]))
        if op is CONCAT:
            return ''.join (args)
        elif op is STAR:
            return '(' + args[0] + ')*'
...
class nfa_dfa_converter:
    ...
    def _epsilon_closure (self, state, visited):
        ...
        if len(visited) == len(adjoined):
            ...
            r = {}
            for s in list(set_difference (adjoined, visited).keys()):
    ...
    def minimize (self):
        ...
        # partition
        non_final = set_difference (make_set (list(t.keys())), final)
        part = [list(final.keys()), list(non_final.keys())]
        last_size = len(part)
        ...
        while 1:
            ...
            s = {}
            crib = {}
            for fs, trans in list(t.items()):
                s2 = {}
                for ch, ts in list(trans.items()):
                    part_num = m[ts]
                    s2[(ch,part_num)] = None
                s2 = list(s2.keys())
                s2.sort()
            ...
            new_part = list(map (lambda x,c=crib: c[x], s))
        ...
        for i in range(len(part)):
            ...
            new_trans = []
            for (ch, ts) in list(old_trans.items()):
        ...
        return new_dfa, list(new_finals.keys())
...
class Test_hello1(unittest.TestCase):
    def test_hello1(self):
        #单符号
        print("----------------------单符号")
        matcher = regexp_machine_1('a')
        print(matcher.feed("a"))
        print(matcher.feed("b"))

        #链接
        print("----------------------链接")
        matcher = regexp_machine_1('ab')
        print(matcher.feed("ab"))
        print(matcher.feed("ba"))

        #闭包
        print("----------------------闭包")
        matcher = regexp_machine_1('a*')
        print(matcher.feed("a"))
        print(matcher.feed("aaa"))
        print(matcher.feed("ba")) #BUG:

        #或
        print("----------------------或")
        matcher = regexp_machine_1('a|b')
        print(matcher.feed("a"))
        print(matcher.feed("b")) #BUG:
        print(matcher.feed("c"))

        #括号组合
        print("----------------------括号组合")
        matcher = regexp_machine_1('(a|b)*c')
        print(matcher.feed("aaac"))
        print(matcher.feed("babc"))
        print(matcher.feed("ccc"))
