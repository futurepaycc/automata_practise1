/* 
来源: https://www.geeksforgeeks.org/program-implement-nfa-epsilon-move-dfa-conversion/
运行: gcc -o enfa2dfa1 enfa2dfa1.c && ./enfa2dfa1
 */
...
void Display_DFA(int last_index, struct DFA *dfa_states, char *DFA_TABLE[][symbols]) {
    ...
    for (i = 1; i < last_index; i++) printf("%s, ", &dfa_states[i].states);
    printf("\n");
...
int main() {
    ...
    printf("\n STATES OF NFA :\t\t");
    for (i = 0; i < states; i++) printf("%c, ", (char)(65 + i));
    printf("\n");
    ...
    while (ind != -1) {
        ...
        if (ind != -1) strcpy(buffer, &dfa_states[++start_index].states);
        zz++;
    }
